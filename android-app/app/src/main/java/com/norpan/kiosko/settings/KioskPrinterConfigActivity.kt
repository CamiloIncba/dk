...
import com.norpan.kiosko.databinding.ActivityPrinterConfigKioskBinding
...
/**
 * Activity para configurar la impresora del Kiosko.
 * Solo muestra la impresora de tickets de cliente (POS80-CX 80mm).
 */
class KioskPrinterConfigActivity : AppCompatActivity() {

    private lateinit var binding: ActivityPrinterConfigKioskBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityPrinterConfigKioskBinding.inflate(layoutInflater)
    ...
    private fun setupUI() {
        ...
        binding.btnKioskSelect.setOnClickListener {
            checkBluetoothPermissionsAndShowDevices { device ->
                PrinterConfig.PRINTER_MAC_ADDRESS = device.address
                PrinterConfig.kioskPrinterName = device.name ?: "Desconocida"
                loadCurrentConfig()
                Toast.makeText(this, "Impresora kiosko configurada", Toast.LENGTH_SHORT).show()
            }
        }

        binding.btnKioskTest.setOnClickListener {
            testPrinter(PrinterConfig.PRINTER_MAC_ADDRESS, "KIOSKO", 48)
        }

        binding.switchKioskEnabled.setOnCheckedChangeListener { _, isChecked ->
            PrinterConfig.kioskPrinterEnabled = isChecked
        }

        binding.switchAutoCustomer.setOnCheckedChangeListener { _, isChecked ->
            PrinterConfig.autoPrintCustomerTicket = isChecked
        }
    }

    private fun loadCurrentConfig() {
        val kioskMac = PrinterConfig.PRINTER_MAC_ADDRESS
        val kioskName = PrinterConfig.kioskPrinterName

        if (kioskMac.isNotBlank()) {
            binding.tvKioskPrinterName.text = "Impresora: $kioskName"
            binding.tvKioskPrinterMac.text = "MAC: $kioskMac"
            binding.tvKioskStatus.text = "Verificando..."
            setIndicatorColor(binding.kioskIndicator, 0xFF9E9E9E.toInt())
        } else {
            binding.tvKioskPrinterName.text = "Impresora: -"
            binding.tvKioskPrinterMac.text = "MAC: -"
            binding.tvKioskStatus.text = "No configurada"
            binding.tvKioskStatus.setTextColor(0xFF666666.toInt())
            setIndicatorColor(binding.kioskIndicator, 0xFF9E9E9E.toInt())
        }

        binding.switchKioskEnabled.isChecked = PrinterConfig.kioskPrinterEnabled
        binding.switchAutoCustomer.isChecked = PrinterConfig.autoPrintCustomerTicket
    }
        ...
        lifecycleScope.launch {
            ...
            withContext(Dispatchers.Main) {
                if (printResult.isSuccess) {
                    Toast.makeText(
                        this@KioskPrinterConfigActivity,
                        "✅ Impresión exitosa",
                        Toast.LENGTH_SHORT
                    ).show()
                } else {
    ...
    private fun buildTestTicket(printerName: String, width: Int): ByteArray {
        ...
        EscPosPrinter.printLn(out, "=".repeat(width.coerceAtMost(48)))
        EscPosPrinter.printLn(out, "")
    ...
    private suspend fun checkPrinterStatus() {
        val kioskMac = PrinterConfig.PRINTER_MAC_ADDRESS
        if (kioskMac.isNotBlank()) {
            val kioskResult = BluetoothPrinterManager.diagnose(kioskMac)
            withContext(Dispatchers.Main) {
                updateIndicator(
                    indicator = binding.kioskIndicator,
                    statusText = binding.tvKioskStatus,
                    result = kioskResult
                )
            }
        }
